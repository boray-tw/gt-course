...
% reference formats
\usepackage{cleveref}
% gray hyperlinks n supplement
% ref: https://tex.stackexchange.com/a/39739
% ref: https://tex.stackexchange.com/a/133095
\makeatletter
\def\grayifsupp#1{
  \def\@targetenvir{supplement}%
  \ifx\@targetenvir\@currenvir
    \textup{%
...
% define a format for both capital and lowercase variants
\newcommand{\defformat}[3]{%
  \def\envname{#1}%
  \def\capitalprefix{#2}%
  \def\lowercaseprefix{#3}%
  % in \crefformat and \Crefformat, #1 is the counter;
  % #2 is counter prefix; #3 is counter suffix.
  \crefformat{tcb@cnt@\envname}{\grayifsupp{\capitalprefix~#2#1#3}}%
  \Crefformat{tcb@cnt@\envname}{\grayifsupp{\lowercaseprefix~#2#1#3}}%
}%
\makeatother
% TCB theorem environments
\defformat{example}{Ex.}{ex.}
\defformat{proposition}{Prop.}{prop.}
\defformat{theorem}{Thm.}{thm.}
\defformat{principle}{Princ.}{princ.}
\defformat{definition}{Def.}{def.}
\defformat{corollary}{Cor.}{cor.}
\defformat{remark}{Rem.}{rem.}
...
\defformat{figure}{Fig.}{fig.}
\defformat{subfigure}{Fig.}{fig.}
% the other references
\defformat{equation}{Eq.}{eq.}
\defformat{table}{Table}{table}
