...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{.3\textwidth}
    \centering
    % TODO: TikZ version
    \includegraphics[width=\textwidth]{con-complement-after}
...
\supp{The complexity of finding a chromatic number is an open problem. It is likely NP-complete or NP-hard, but we couldn't find a way to reduce it to an NP-complete problem.}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{.2\textwidth}
    \centering
    % TODO: TikZ version
    \includegraphics[width=.8\textwidth]{con-rooted-tree}
  ...
  \begin{subfigure}[t]{.2\textwidth}
    \centering
    % TODO: TikZ version
    \includegraphics[width=\textwidth]{con-unrooted-tree}
...
\begin{enumerate}
  ...
  \item See \cref{thm:tree-tree-stat} (the same definition as the previous one) and its proofs from \cpageref{thm:tree-tree-stat} for more info.
\end{enumerate}
...
For \cref{def:con-subgraph}, in other words, if $S \subseteq G$, all vertices and edges in $S$ are in $G$ without introducing new vertices or edges. In \cref{def:con-induced-subgraph} in \cpageref{def:con-induced-subgraph}, we will cover induced subgraphs, which pose a constraint on edges.
...
\reusefigure[htbp]{fig:intro-special-functions}
...
$C_3$ is a subgraph of $K_5$. In a $K_5$, there are $\C{5}{3} = 10$ (combination) distinct $C_3$. However, in $K_{2,3}$, there is no $C_3$, which is an odd-length cycle. See \cref{thm:con-bipartite-odd-cycle} in \cpageref{thm:con-bipartite-odd-cycle}, comment \ref{enum:con-bipartite-odd} in \cpageref{enum:con-bipartite-odd}, and \cref{rem:intro-equal-sets} in \cpageref{rem:intro-equal-sets} for the definition of bipartite graphs.
...
\Cref{prop:con-component} yields that to have a connected graph with $n$ vertices (there is only a component; solve $n - k = 1$ for $k$), you need \textbf{at least} $n - 1$ edges. A \textbf{tree} is a commonly seen connected graph with $n$ vertices and $n - 1$ edges.
...
\begin{theorem}{}{con-eulerian-component-degree}
  A graph $G$ is Eulerian if and only if \ul{it has \textbf{at most} one nontrivial component} and \ul{all its vertices have \textbf{even} \textbf{degree}}. % use isolated bolded words to overcome nested style issues.
\end{theorem}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{.4\textwidth}
    \centering
    \begin{tikzpicture}
      \node[draw, circle, fill=blue!25!white, inner sep=.15cm] (1) at (0, 2) {$v$};
      \draw[ultra thick] (1) -- (2, 2) -- (2, 0) -- (0, 0) -- (1);
      \draw[ultra thick, red] (1) -- (2, 0);
    \end{tikzpicture}
  ...
  \begin{subfigure}[t]{.75\textwidth}
    % TODO: TikZ version
    \centering
    \includegraphics[width=\textwidth]{con-bigraph-edges-switching}
...
Continuing form \cref{def:con-digraph}, $uv$ and $vu$ edges refer to different edges. \supp{In some programs, the direction of head and tail may be reversed because the programmer thinks that the starting end is the head, and the ending end is the tail.}
...
\begin{figure}[htbp]
  % TODO: TikZ version
  \centering
  \includegraphics[width=.5\textwidth]{con-digraph-cycle}
...
\begin{figure}[htbp]
  % TODO: TikZ version
  \centering
  \includegraphics[width=.75\textwidth]{con-digraph-adj-inc-matrices}
...
\begin{figure}[htbp]
  % TODO: TikZ version
  \centering
  \includegraphics[width=.4\textwidth]{con-strong-component}
...
\begin{figure}[htbp]
  % TODO: TikZ version
  \centering
  \includegraphics[width=.4\textwidth]{con-digraph-cycle}
...
\begin{figure}[htbp]
  % TODO: TikZ version
  \centering
  \includegraphics[width=.25\textwidth]{con-tournament}
...
\begin{enumerate*}
  \item Let $x$ be a vertex in a tournament $T$. \supp{Since we are not proving whether a vertex is king or not, we cannot assume that a vertex is a king in the first place.}
...
\begin{figure}[htbp]
  % TODO: TikZ version
  \centering
  \includegraphics[width=.4\textwidth]{con-tournament-king}
